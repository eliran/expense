...
module.exports = class ExpenseController {
  ...
  getUserExpenses(req){
    ...
    if ( filter.query ) query = query.where([ '(description LIKE :query OR comment LIKE :query)', { query: '%' + filter.query + '%' } ])
    ...
    return query.order('dateTime', true).exec().then(function(expenses){
      return Response.ok(expenses.map(function(expense){
  ...
  allowedAccess(req){
    ...
    var session = req.session || {}
      , params = req.params || {}
    return parseInt(session.userId) === parseInt(params.userId) || session.role === 'admin'
  }
...
function prepareExpenseRecord(record){
  ...
  , dateTime: dateTimeToUnix(record.dateTime)
  }
}
