...
module.exports = class UserController {
  ...
  getSession(req){
    var self = this
    if ( !req || !req.session || !req.session.userId ) return Promise.resolve(Response.unauthorized())
  ...
  manageUsersAllowed(req, allowSelf){
    if ( allowSelf ) {
  ...
  getUser(req){
    if ( !this.manageUsersAllowed(req, true) ) return Promise.resolve(Response.unauthorized())
